...
goto
out_unlock;    /* No processes for this user */
...
new->
gid = new->egid = new->sgid = new->fsgid = gid;
...
SYSCALL_DEFINE3(getresuid, uid_t
__user *, ruid,
uid_t __user
*, euid,
uid_t __user
...
SYSCALL_DEFINE3(getresgid, gid_t
__user *, rgid,
gid_t __user
*, egid,
gid_t __user
...
->nodename, tmp, len);
memset(u
->nodename + len, 0, sizeof(u->nodename) - len);
errno = 0;
}
up_write(&uts_sem);
return
...
->domainname, tmp, len);
memset(u
->domainname + len, 0, sizeof(u->domainname) - len);
errno = 0;
}
up_write(&uts_sem);
return
...
int getrusage(struct task_struct *p, int who, struct rusage __user
...
error = put_user(me->pdeath_signal,(
int __user
*)arg2);
break;
